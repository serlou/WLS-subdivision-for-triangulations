...
t_x = linspace(min(x), max(x), 1000);  % Evaluation points in x
t_y = linspace(min(y), max(y), 1000);  % Evaluation points in y
...
% Plot the original data and the fitted 2D B-spline surface
h=utils().new_figure();
scatter3(x,y, z,'filled');
hold on;
surf( y_grid,x_grid, fitted_surface, 'EdgeColor', 'none');
%hold off;
utils().apply_format(h,false,4);
if save_results
    utils().my_saveas(h,'graphics/bsplines.eps');
end